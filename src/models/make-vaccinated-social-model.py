...
def main():
    ...
    X = social_data.drop(columns=['state', 
                                ...
                                'total_percent_hosp',
                                'percent_vaccinated'])
    y = social_data['percent_vaccinated']
...
# Pretty consistent, MAE=0.059, MSE=0.0054 ~ MSE somewhat inconsistent
def get_svr_model():
    return make_pipeline(
        SVR(kernel='poly', C=2.0)
    )

# Pretty consistent, MAE=0.054, MSE=0.0049
def get_kneighbors_model():
    return make_pipeline(
        KNeighborsRegressor(n_neighbors=8)
    )

# Pretty consistent, MAE=0.061, MSE=0.0068
def get_decision_tree_model():
    return make_pipeline(
        DecisionTreeRegressor(random_state=0, splitter='random')
    )

# Pretty consistent, MAE=0.044, MSE=0.0034
def get_random_forest_model():
    return make_pipeline(
        RandomForestRegressor(n_estimators=100, max_depth=3, min_samples_leaf=10)
    )

# Pretty consistent, MAE=0.048, MSE=0.0038
...
# Pretty consistent, MAE=0.054, MSE=0.0048
